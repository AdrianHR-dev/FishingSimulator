...
CharacterStates TestDialogue::getCurrState(PlayerData* pd)
{
	if (progress == 1)
	{
		return testText.returnCharState(pd, progress);
	}

	else
	{
		return testText.returnCharState(pd, ((progress - 1)/2)+1); //((N-1)/2) + 1 eg. n = 3, input = 2; n = 7, input = 4
	}
}

bool TestDialogue::DialogueUpdate(PlayerData* pd)
{
	progress = progress + 2;
	...
	case(1):
	{
		name.setString(testText.testDialogue1.at(pd->tasks).at(progress));
		dialogue.setString(testText.testDialogue1.at(pd->tasks).at(progress + 1));
